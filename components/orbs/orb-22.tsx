"use client";
/*
 * Ported from orbkit (WebGL/GLSL) to WebGPU/WGSL for shadercn.
 * Original: https://github.com/zzzzshawn/orbkit
 */
import { ShaderOrb } from "./orbkit-core-wgpu";
import type { OrbVariant, ShaderOrbProps } from "./orbkit-core-wgpu";

const VECTORS_FRAG = `
const STEPS: i32 = 70;
const TURB: i32 = 7;
const AA: i32 = 1;
// Volume-reactive values, resolved once per fragment in main().
var<private> vectorsTurb: f32;
var<private> vectorsExposure: f32;
var<private> vectorsGlow: f32;

fn vectorsRender(fragCoord: vec2f) -> vec3f {
  var animTime: f32 =uP_speed; // integrated clock: cell flicker phase
  var wander: f32 =uP_wander;  // integrated clock: axis drift

  var uv: vec2f = (2.0 * fragCoord - uRes) / min(uRes.x, uRes.y);
  var ro: vec3f = vec3f(0.0, 0.0, uP_camDist);
  var rd: vec3f = normalize(vec3f(uv, -uP_focal));

  // the wandering rotation axis — unit by construction, which is what
  // makes the 90-degree Rodrigues below exact
  var axis: vec3f = normalize(sin(wander + vec3f(0.0, 2.0, 4.0)));

  var acc: vec3f = vec3f(0.0);

  // transmittance carried front-to-back — near streaks veil far ones
  var T: f32 =1.0;

  // march only the span the envelope can light, as in orb-01
  var z: f32 =max(uP_camDist - uP_envRadius * 1.3, 0.0);
  var zEnd: f32 =uP_camDist + uP_envRadius * 1.3;

  for (var it: i32 =0; it < STEPS; it = it + 1) {
    var p: vec3f = ro + rd * z;

    /*
      THE ORB IS THE OBJECT — pull the sample toward the sphere's shell
      before the field ever sees it. At hug 0 this is the raw 3D tangle;
      at 1 the field is purely angular, painted on the ball's skin. The
      guard keeps normalize() defined through the centre.
    */
    var rl: f32 =max(length(p), 1e-3);
    var q: vec3f = mix(p, p / rl * uP_envRadius, uP_hug);

    // the exact 90-degree rotation about the wandering axis; v stays
    // clean, a takes the turbulence — the fork is the original's v=a=...
    var v: vec3f = dot(axis, q) * axis + cross(axis, q);
    var a: vec3f = v;

    // cell-quantized turbulence: every ceil() lattice cell flickers on
    // its own phase
    for (var j: i32 =0; j < TURB; j = j + 1) {
      var dj: f32 =f32(j) + 3.0;
      a += vectorsTurb * sin(ceil(a * dj) - animTime).yzx / dj;
    }

    // the density product — turbulent detail times clean streak surfaces
    var d: f32 =uP_stepScale * length(sin(a * a)) * sqrt(length(v * sin(v.yzx)));
    d = max(d, 1e-4);

    /*
      The march's own colour code, from the listing: red constant, green
      by step index, blue by depth into the ball — with the two ramps
      exposed. The CLAMP is load-bearing as everywhere: one grazing step
      would own the whole 70-step sum at some phases. Green gets twice the
      clamp headroom: its ramp runs to STEPS (70) where red is fixed at 9,
      and an equal clamp would crush the step gradient first.
    */
    var w: vec3f = vec3f(9.0, f32(it) * uP_hueStep, (z - uP_camDist + uP_envRadius) * uP_hueDepth) / d;
    w = min(w, vec3f(uP_stepClamp) * vec3f(1.0, 2.0, 1.0));

    /*
      Normalize the clamped weight back to family units (a ceiling of ~20,
      like the sibling orbs). This orb's raw weights run in the hundreds —
      the golfed knee was 6e4 — and without this one line the clamp value
      leaks into total energy, so Exposure, Body fill and Diffusion would
      all change meaning whenever the clamp moves. Normalized, stepClamp
      is a pure dynamic-range knob: low flattens the streaks, high lets
      the 1/d spikes whiten.
    */
    w *= 20.0 / max(uP_stepClamp, 1.0);

    /*
      A few vectors GLOW. Cells of the clean rotated frame are hashed, and
      each cell's hash cycles against the clock so only a small fraction
      (uP_glowFew of the cycle) are hot at any moment. Where a hot cell
      meets a streak null, the same 1/d spike is re-read on a far higher
      ceiling than the step clamp — deliberately bypassing it — and pushed
      as warm-white light. The two smoothsteps make a triangle window, so
      each glow blooms and fades instead of popping at the cycle wrap.
    */
    var few: f32 =max(uP_glowFew, 1e-3);
    var vc: vec3f = ceil(v * 2.0);
    var hcell: f32 =hash(vc.xy + vc.z * vec2f(7.31, 3.17));
    var cyc: f32 =fract(hcell + animTime * 0.05);
    var sel: f32 =smoothstep(1.0 - few, 1.0 - 0.5 * few, cyc) * smoothstep(1.0, 1.0 - 0.5 * few, cyc);
    // QUADRATIC in 1/d, unlike the linear base weight — the glow hugs the
    // filament core and falls off fast, a hot wire rather than a lit sector
    w += vec3f(1.0, 0.96, 0.88) * min(0.08 / (d * d), 500.0) * sel * vectorsGlow;

    // envelope: plateau through the ball, cut 12% past the radius so the
    // analytic silhouette in main() still has emission left to cut
    var env: f32 =smoothstep(uP_envRadius * 1.12, uP_envRadius * uP_envCore, length(p));
    w = (w + uP_fill) * env;

    acc += T * w;
    T *= exp(-dot(w, vec3f(0.299, 0.587, 0.114)) * uP_scatter);

    z += d;
    if (T < 0.004 || z > zEnd) { break; }
  }

  return acc;
}

fn orbMain(fragCoord: vec2f, uv: vec2f) -> vec4f {
  vectorsTurb = uP_turb * (1.0 + 0.5 * uInput);
  vectorsExposure = uP_exposure * (1.0 - 0.35 * uOutput);
  // the glows flare when the agent speaks
  vectorsGlow = uP_glow * (1.0 + 0.8 * uOutput);

  var acc: vec3f = vec3f(0.0);
  acc = vectorsRender(fragCoord);

  // tanh tone map per channel — the envelope and transmittance change the
  // accumulator's scale, so the golfed /6e4 knee is a tunable here
  var col: vec3f = tanh3(acc / max(vectorsExposure, 1.0));
  col = pow(clamp(col, vec3f(0.0), vec3f(1.0)), vec3f(uP_contrast));

  // saturation about luminance, then the tint
  var lum: f32 =dot(col, vec3f(0.299, 0.587, 0.114));
  col = mix(vec3f(lum), col, uP_saturation);
  col *= uC_tint;

  // alpha from the brightest channel, not luminance — a deep blue tail
  // has low luminance but must not go transparent
  var peak: f32 =max(col.r, max(col.g, col.b));
  var a: f32 =clamp(peak * uP_alphaGain, 0.0, 1.0);

  // Analytic silhouette — identical construction to orb-01: exact
  // ray-to-centre distance against the radius, colour AND alpha.
  var mrd: vec3f = normalize(vec3f(orbUV(), -uP_focal));
  var closest: f32 =length(cross(vec3f(0.0, 0.0, uP_camDist), mrd));
  var band: f32 =mix(0.35, 0.012, clamp(uP_edge, 0.0, 1.0));
  var mask: f32 =1.0 - smoothstep(uP_envRadius * (1.0 - band), uP_envRadius * 1.005, closest);
  col *= mask;
  a *= mask;

  // safety taper at the frame boundary — colour as well as alpha
  var r2d: f32 =length(orbUV());
  var fade: f32 =1.0 - smoothstep(uP_edgeFade, 1.0, r2d);
  col *= fade;
  a *= fade;

  // Emitted light, so rgb is already premultiplied — do NOT scale by alpha
  // again (see the same note in orb-31).
  return vec4f(col, a);
}
`;

export const orb22Orb: OrbVariant = {
  colors: [{ default: "#ffffff", key: "tint", label: "Tint" }],
  frag: VECTORS_FRAG,
  key: "orb-22",
  label: "ORB-22",
  note: "field lines swirling around the ball about a wandering axis",
  params: [
    {
      default: 0.5,
      integrate: true,
      key: "speed",
      label: "Anim speed",
      max: 10,
      min: 0.015,
      step: 0.05,
    },
    {
      default: 0.12,
      integrate: true,
      key: "wander",
      label: "Axis wander",
      max: 3,
      min: 0,
      step: 0.015,
    },
    {
      default: 7,
      key: "camDist",
      label: "Camera distance",
      max: 50,
      min: 1,
      step: 0.3,
    },
    {
      default: 2.25,
      key: "focal",
      label: "Lens",
      max: 15,
      min: 0.15,
      step: 0.1,
    },
    {
      default: 0.8,
      key: "hug",
      label: "Surface hug",
      max: 3,
      min: 0,
      step: 0.015,
    },
    {
      default: 0.6,
      key: "turb",
      label: "Cell shimmer",
      max: 5,
      min: 0,
      step: 0.03,
    },
    {
      default: 0.07,
      key: "stepScale",
      label: "Step scale",
      max: 1.5,
      min: 0.005,
      step: 0.01,
    },
    {
      default: 0.12,
      key: "hueStep",
      label: "Step hue",
      max: 10,
      min: 0,
      step: 0.03,
    },
    {
      default: 1.4,
      key: "hueDepth",
      label: "Depth hue",
      max: 10,
      min: 0,
      step: 0.03,
    },
    {
      default: 1.4,
      key: "glow",
      label: "Vector glow",
      max: 10,
      min: 0,
      step: 0.05,
    },
    {
      default: 0.12,
      key: "glowFew",
      label: "Glow density",
      max: 1.5,
      min: 0,
      step: 0.01,
    },
    {
      default: 2.6,
      key: "envRadius",
      label: "Envelope radius",
      max: 15,
      min: 0.15,
      step: 0.1,
    },
    {
      default: 0.88,
      key: "envCore",
      label: "Envelope core",
      max: 1.02,
      min: 0.3,
      step: 0.01,
    },
    {
      default: 0.15,
      key: "fill",
      label: "Body fill",
      max: 100,
      min: 0,
      step: 0.3,
    },
    {
      default: 800,
      key: "stepClamp",
      label: "Step clamp",
      max: 5000,
      min: 3,
      step: 30,
    },
    {
      default: 0.01,
      key: "scatter",
      label: "Diffusion",
      max: 0.5,
      min: 0,
      step: 0.003,
    },
    {
      default: 260,
      key: "exposure",
      label: "Exposure",
      max: 5000,
      min: 1.5,
      step: 15,
    },
    {
      default: 1.3,
      key: "contrast",
      label: "Contrast",
      max: 15,
      min: 0.15,
      step: 0.1,
    },
    {
      default: 1.15,
      key: "saturation",
      label: "Saturation",
      max: 4,
      min: 0,
      step: 0.02,
    },
    {
      default: 2,
      key: "alphaGain",
      label: "Alpha gain",
      max: 15,
      min: 0.05,
      step: 0.1,
    },
    {
      default: 1,
      key: "edge",
      label: "Edge sharpness",
      max: 1,
      min: 0,
      step: 0.01,
    },
    {
      default: 0.98,
      key: "edgeFade",
      label: "Halo falloff",
      max: 3,
      min: 0.1,
      step: 0.015,
    },
  ],
  // the tint carries the at-a-glance read, as in chords: neutral at rest,
  // cooled while searching, warmed while answering
  stateColors: {
    idle: { tint: "#ffffff" },
    speaking: { tint: "#ffd9c4" },
    thinking: { tint: "#c3d2ff" },
  },
  /*
    The states are staged on this orb's two best levers, both phase-safe
    integrated clocks: the AXIS WANDER (the whole field re-orients as the
    axis moves) and the cell-flicker speed. glowFew is the third lever —
    how many of the hot-wire vectors are lit at once.
  */
  statePresets: {
    idle: {
      alphaGain: 2,
      exposure: 260,
      glow: 1.3,
      glowFew: 0.12,
      scatter: 0.01,
      speed: 0.4,
      turb: 0.55,
      wander: 0.1,
    },
    speaking: {
      alphaGain: 2.5,
      exposure: 170,
      glow: 2.4,
      glowFew: 0.22,
      scatter: 0.0075,
      speed: 2.2,
      turb: 0.85,
      wander: 0.3,
    },
    thinking: {
      alphaGain: 2.1,
      exposure: 230,
      glow: 1.7,
      glowFew: 0.07,
      scatter: 0.0095,
      speed: 1.2,
      turb: 0.7,
      wander: 0.6,
    },
  },
};

export type Orb22Props = Omit<ShaderOrbProps, "variant">;

export const Orb22 = ({ size = 280, ...rest }: Orb22Props) => (
  <ShaderOrb variant={orb22Orb} size={size} {...rest} />
);

export default Orb22;
